// TrustSfer REST API. The Express app is built and seeded once at module
// load and exported as a singleton — the production entry point
// (server/index.js) calls `app.listen`, and the Vite dev server mounts
// the same `app` as middleware so the API and the front-end always run
// in the same Node process. There is no second port to start.
